---
title: Projects
layout: page
permalink: /projects/
---

Github: https://github.com/joshuammar/

- [AWS S3 Cross-Account Cross-Region Replication - Disaster Recovery data strategy](/project-pages/s3-disaster-recovery)

- [CircleCI Orb Development - Creating modular build and deploy jobs defined once, used everwhere](/project-pages/circle-ci)

- [AWS S3 Malware Scanning - Real-time Malware analysis on static file uploads](/project-pages/aws-s3-malware)

- [Cross-Cloud (Azure Blob->AWS S3) Data Migration using AWS DataSync and Boto3 Python Automation](/project-pages/azure-blob-to-aws-s3)

- [AWS CDK Lambda/Application Migration](/project-pages/aws-cdk-lambda-migration)
    - NX Webpack build orchestration via pnpm
    - Leveraging CDK Constructs
    - Continuous Integration/Continous Deploy

- This Site! https://github.com/joshuammar/personal-website